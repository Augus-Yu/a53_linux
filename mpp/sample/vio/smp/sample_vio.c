...
HI_S32 SAMPLE_VIO_ViOnlineVpssOnlineRoute(HI_U32 u32VoIntfType)
{
    ...
    /*config vo*/
    SAMPLE_OV9734_COMM_VO_GetDefConfig(&stVoConfig); 
...
HI_S32 SAMPLE_VIO_ViOnlineVpssOnlineRoute_ov6946(HI_U32 u32VoIntfType)
{
    HI_S32             s32Ret;
    ...
    /*config vo*/
    SAMPLE_OV6946_COMM_VO_GetDefConfig(&stVoConfig); 
    stVoConfig.enDstDynamicRange = enDynamicRange;
    if (1 == u32VoIntfType)
    {
        stVoConfig.enVoIntfType = VO_INTF_BT1120;
...
HI_S32 SAMPLE_VIO_ViOfflineVpssOfflineRoute(HI_U32 u32VoIntfType)
{
    HI_S32             s32Ret;
...
HI_S32 SAMPLE_VIO_ViWdrSwitch(HI_U32 u32VoIntfType)
{
    ...
    enMastPipeMode = stViConfig.astViInfo[s32WorkSnsId].stPipeInfo.enMastPipeMode;
...
HI_S32 SAMPLE_VIO_ViResoSwitch(HI_U32 u32VoIntfType)
{
    ...
    if (SONY_IMX290_MIPI_2M_30FPS_12BIT != SENSOR0_TYPE)
    {
        SAMPLE_PRT("Sensor Not Support!\n");
        return HI_SUCCESS;
    }
    ...
    s32Ret = HI_MPI_VENC_GetChnAttr(VencChn[0], &stChnAttr);
    if (HI_SUCCESS != s32Ret)
    {
    ...
    printf("switch to 720P wdr mode========\n");
    getchar();
    ...
    stViConfig.astViInfo[s32WorkSnsId].stSnsInfo.enSnsType = SONY_IMX290_MIPI_1M_30FPS_12BIT_WDR2TO1;
    ...
    s32Ret = HI_MPI_VENC_StopRecvFrame(VencChn[0]);
    if (HI_SUCCESS != s32Ret)
    {
    ...
    s32Ret = HI_MPI_VENC_SetChnAttr(VencChn[0], &stChnAttr);
    if (HI_SUCCESS != s32Ret)
    {
    ...
    s32Ret = HI_MPI_VENC_StartRecvFrame(VencChn[0], &stRecvParam);
    if (HI_SUCCESS != s32Ret)
    {
    ...
    printf("switch to 1080P wdr mode========\n");
    getchar();
