...
static  HI_S32 iMax2(HI_S32 a, HI_S32 b)
{
    {
        if (a > b)
        {
...
HI_S32 HI_SCENE_Set3DNR(VI_PIPE ViPipe, const HI_SCENE_3DNR_S *pst3Nrx)
{
    ...
    /*NRc*/
    stNrx.stNRXParam_V1.stNRXManual.stNRXParam.NRc.MODE = pst3Nrx->NRc.MODE;
    stNrx.stNRXParam_V1.stNRXManual.stNRXParam.NRc.PRESFC= pst3Nrx->NRc.PRESFC;
...
HI_S32 HI_SCENE_SetDynamicVencBitrate(VI_PIPE ViPipe, HI_U64 u64Iso, HI_U64 u64LastIso, HI_U8 u8Index)
{
    ...
    s32Ret = HI_MPI_VI_GetStitchGrpAttr(StitchGrp, &stStitchGrpAttr);
    CHECK_SCENE_RET(s32Ret);
    if (ViPipe != 0 && stStitchGrpAttr. bStitch == HI_TRUE &&
