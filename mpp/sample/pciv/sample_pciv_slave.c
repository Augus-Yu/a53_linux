...
#define     PCIE_PFAHB_BADE      0x9f800000
...
HI_VOID * SamplePciv_SendVencThread(HI_VOID *p)
{
    ...
    while (pstCtx->bThreadStart)
    {
        ...
        for (i=0; i<pstCtx->s32VencCnt; i++)
        {
            ...
            {
                printf("venc stream local buffer not enough,chn:%d, %d < %d+%u \n",
...
HI_S32 SamplePcivInitVi(SAMPLE_PCIV_MSG_INIT_VI_S *pstViArgs)
{
    ...
    VI_CHN             ViChn               = 0;
    HI_S32             s32SnsId            = 0;
    PIC_SIZE_E         enPicSize           = pstViArgs->stPicVbAttr.enDispPicSize;
    ...
    *************************************************/
    ...
    g_stViConfig.astViInfo[s32SnsId].stSnsInfo.s32BusId           = 1;
    ...
    g_stViConfig.astViInfo[s32SnsId].stChnInfo.enPixFormat        = pstViArgs->stPicVbAttr.enPixelFormat;
    g_stViConfig.astViInfo[s32SnsId].stChnInfo.enDynamicRange     = pstViArgs->stPicVbAttr.enDynamicRange;
    g_stViConfig.astViInfo[s32SnsId].stChnInfo.enVideoFormat      = pstViArgs->stPicVbAttr.enVideoFormat;
    g_stViConfig.astViInfo[s32SnsId].stChnInfo.enCompressMode     = pstViArgs->stPicVbAttr.enCompressMode;
