...
void* SAMPLE_COMM_AUDIO_AiExtResProc(void* parg)
{
    ...
    hRes = HI_Resampler_Create(pstAiCtl->enInSample, pstAiCtl->enOutSample, 1);

    ps16OutBuf = malloc(HI_Resampler_GetMaxOutputNum(hRes, pstAiCtl->u32PerFrame) * sizeof(HI_S16) + 2);
        ...
        if (FD_ISSET(AiFd, &read_fds))
        {
            ...
            if (1 != s32Mulit)
            {
                if (0 != u32CacheCount)
                {
                    ...
                    u32OutSample = HI_Resampler_Process(hRes, s16Cache, s32Mulit, ps16OutBuf);
                ...
                u32OutSample = HI_Resampler_Process(hRes, (HI_S16*)(stFrame.u64VirAddr[0] + (s32Mulit - u32CacheCount)*sizeof(HI_S16)), u32ProcFrame, ps16OutBuf);
                (HI_VOID)fwrite(ps16OutBuf, 2, u32OutSample, pstAiCtl->pfd);
            ...
            {
                u32OutSample = HI_Resampler_Process(hRes, (HI_S16*)stFrame.u64VirAddr[0], pstAiCtl->u32PerFrame, ps16OutBuf);
                (HI_VOID)fwrite(ps16OutBuf, 2, u32OutSample, pstAiCtl->pfd);
            }
...
HI_S32 SAMPLE_AUDIO_AiToExtResample(HI_VOID)
{
    ...
    ********************************************/
    ...
    /********************************************
      step 3: Create thread to resample.
    ********************************************/
