...
static HI_VOID GetBuiltInExpanderBlc(const hi_isp_cmos_split *sns_split, HI_U16 *pu16SensorBlc, HI_U16 *pu16ExpanderBlc)
{
    HI_U8 i, j;
    ...
    for (i = 1; i < ISP_SPLIT_POINT_NUM + 1; i++) {
        au8X[i]  = sns_split->split_point[i - 1].x;
        au16Y[i] = sns_split->split_point[i - 1].y;
    }
    ...
    for (j = 0; j < ISP_BAYER_CHN_NUM; j++) {
        ...
        pu16ExpanderBlc[j] = ((HI_U16)LinearInterpol(pu16SensorBlc[j], au8X[u8IndexLow], au16Y[u8IndexLow], au8X[u8IndexUp], au16Y[u8IndexUp], u8Shift)) >> (1 + u8Shift);
    }
}
...
static HI_VOID BE_BlcDynaRegs_Linear(VI_PIPE ViPipe, ISP_BE_BLC_CFG_S *pstBeBlcCfg, ISP_BLACKLEVEL_S *pstBlackLevel)
{
    ...
    for (j = 0; j < ISP_BAYER_CHN_NUM; j++) {
        ...
        pstBeBlcCfg->stSplitBlc.stUsrRegCfg.au16Blc[j] = 0; /* split */
        pstBeBlcCfg->stLogLUTBlc.stUsrRegCfg.au16Blc[j] = pstBlackLevel->au16RmDiffBlackLevel[j] << 4; /* LogLUT */
        pstBeBlcCfg->stRLscBlc.stUsrRegCfg.au16Blc[j] = pstBlackLevel->au16RmDiffBlackLevel[j] << 2; /* rlsc */
        ...
        pstBeBlcCfg->stPreGammaBlc.stUsrRegCfg.au16Blc[j] = 0; /* pregamma */
    }
...
static HI_VOID BE_BlcDynaRegs_Wdr(VI_PIPE ViPipe, ISP_BE_BLC_CFG_S *pstBeBlcCfg, ISP_BLACKLEVEL_S *pstBlackLevel)
{
    ...
    if (u8WDRModeState == HI_FALSE) { // reg value same as linear mode
        ...
        for (j = 0; j < ISP_BAYER_CHN_NUM; j++) {
            for (i = 0; i < ISP_WDR_CHN_MAX; i++) {
                pstBeBlcCfg->st4DgBlc[i].stUsrRegCfg.au16Blc[j] = pstBlackLevel->au16RmDiffBlackLevel[j] << 2; /* 4DG */
                pstBeBlcCfg->stWdrBlc[i].stUsrRegCfg.au16Blc[j] = pstBlackLevel->au16RmDiffBlackLevel[j] << 2; /* WDR */
            ...
            pstBeBlcCfg->stSplitBlc.stUsrRegCfg.au16Blc[j] = 0; /* split */
            pstBeBlcCfg->stLogLUTBlc.stUsrRegCfg.au16Blc[j] = 0; /* LogLUT */
            pstBeBlcCfg->stRLscBlc.stUsrRegCfg.au16Blc[j] = WDROutBlc >> 6; /* rlsc */
...
static HI_S32 SplitCheckCmosParam(VI_PIPE ViPipe, const hi_isp_cmos_split *split)
{
    ...
    for (i = 0; i < ISP_SPLIT_POINT_NUM; i++) {
        if (split->split_point[i].x > 0x81) {
            ISP_ERR_TRACE("Invalid astSplitPoint[%d].u8X:%d!\n", i, split->split_point[i].x);
            return HI_ERR_ISP_ILLEGAL_PARAM;
        }

        if (split->split_point[i].y > 0x8000) {
            ISP_ERR_TRACE("Invalid astSplitPoint[%d].u16Y:%d!\n", i, split->split_point[i].y);
            return HI_ERR_ISP_ILLEGAL_PARAM;
        }
    }
...
static HI_S32 BE_BlcDynaRegs_BuiltIn(VI_PIPE ViPipe, ISP_BE_BLC_CFG_S *pstBeBlcCfg, ISP_BLACKLEVEL_S *pstBlackLevel)
{
    ...
    if (sns_dft->key.bit1_split) {
        ...
        GetBuiltInExpanderBlc(sns_dft->split, pstBlackLevel->au16RmDiffBlackLevel, au16BlackLevel);
        GetBuiltInExpanderBlc(sns_dft->split, pstBlackLevel->au16BlackLevel, au16ActualBlackLevel);
    } else {
        for (i = 0; i < ISP_BAYER_CHN_NUM; i++) {
    ...
    for (j = 0; j < ISP_BAYER_CHN_NUM; j++) {
        ...
        pstBeBlcCfg->stSplitBlc.stUsrRegCfg.au16Blc[j] = 0; /* split */
        pstBeBlcCfg->stLogLUTBlc.stUsrRegCfg.au16Blc[j] = 0; /* LogLUT */
        pstBeBlcCfg->stRLscBlc.stUsrRegCfg.au16Blc[j] = au16BlackLevel[j]; /* rlsc */
...
static HI_VOID FE_BlcDynaRegs(ISP_FE_BLC_CFG_S *pstFeBlcCfg, ISP_BLACKLEVEL_S *pstBlackLevel)
{
    ...
    for (j = 0; j < ISP_BAYER_CHN_NUM; j++) {
        ...
        pstFeBlcCfg->stFeLscBlc.stUsrRegCfg.au16Blc[j] = pstBlackLevel->au16RmDiffBlackLevel[j] << 2; /* Fe LSC */
        pstFeBlcCfg->stRcBlc.stUsrRegCfg.au16Blc[j] = pstBlackLevel->au16RmDiffBlackLevel[j] << 2; /* RC */
        pstFeBlcCfg->stFeLogLUTBlc.stUsrRegCfg.au16Blc[j] = pstBlackLevel->au16RmDiffBlackLevel[j] << 4; /* Fe LogLUT */
    }
...
HI_S32 ISP_BlcRun(VI_PIPE ViPipe, const HI_VOID *pStatInfo, HI_VOID *pRegCfg, HI_S32 s32Rsv)
{
    ...
    if (sns_black_level->update == HI_TRUE) {
        ...
        if (pstIspCtx->stitch_attr.stitch_enable == HI_TRUE) {
            ...
            for (i = 0; i < ISP_BAYER_CHN_NUM; i++) {
                s16BlkOffset = hi_ext_system_isp_pipe_diff_offset_read(ViPipe, i);
                s32GainK = hi_ext_system_isp_pipe_diff_gain_read(ViPipe, i);
                s32BlcTmp = ((HI_S32)pstBlackLevel->au16BlackLevel[i] * 0x100 / DIV_0_TO_1(s32GainK)) - s16BlkOffset;
                pstBlackLevel->au16BlackLevel[i] = CLIP3(s32BlcTmp, 0, 0xFFF);
            }
        } else {
            isp_sensor_update_blc(ViPipe);
            memcpy(&pstBlackLevel->au16BlackLevel[0], &sns_black_level->black_level[0], ISP_BAYER_CHN_NUM * sizeof(HI_U16));
