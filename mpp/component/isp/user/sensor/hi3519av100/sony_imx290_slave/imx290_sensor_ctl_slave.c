...
extern HI_U32 g_Imx290SalveSensorModeTime[];

extern ISP_SNS_STATE_S *g_pastImx290Slave[ISP_MAX_PIPE_NUM];
...
void imx290slave_init(VI_PIPE ViPipe)
{
    ...
    bInit = g_pastImx290Slave[ViPipe]->bInit;
    enWDRMode = g_pastImx290Slave[ViPipe]->enWDRMode;
    SlaveDev = g_Imx290SlaveBindDev[ViPipe];
    u8ImgMode = g_pastImx290Slave[ViPipe]->u8ImgMode;
    u32Data = g_Imx290SalveSensorModeTime[ViPipe];
    ...
    gstImx290Sync[ViPipe].u32SlaveModeTime = u32Data;
    ...
    if (g_pastImx290Slave[ViPipe]->astRegsInfo[0].stSlvSync.u32SlaveVsTime == 0) {
        gstImx290Sync[ViPipe].u32VsTime = g_astImx290ModeTbl[u8ImgMode].u32InckPerVs;
    } else {
        gstImx290Sync[ViPipe].u32VsTime = g_pastImx290Slave[ViPipe]->astRegsInfo[0].stSlvSync.u32SlaveVsTime;
    }
    ...
    for (i = 0; i < g_pastImx290Slave[ViPipe]->astRegsInfo[0].u32RegNum; i++) {
        imx290slave_write_register(ViPipe, g_pastImx290Slave[ViPipe]->astRegsInfo[0].astI2cData[i].u32RegAddr, g_pastImx290Slave[ViPipe]->astRegsInfo[0].astI2cData[i].u32Data);
    }

    g_pastImx290Slave[ViPipe]->bInit = HI_TRUE;
    return;
}
