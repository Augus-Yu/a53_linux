...
#if !defined(__OV6946_CMOS_H_)
#define __OV6946_CMOS_H_
...
ISP_SNS_STATE_S *g_pastOv6946[ISP_MAX_PIPE_NUM] = { HI_NULL };

#define OV6946_SENSOR_GET_CTX(dev, pstCtx) (pstCtx = g_pastOv6946[dev])
#define OV6946_SENSOR_SET_CTX(dev, pstCtx) (g_pastOv6946[dev] = pstCtx)
#define OV6946_SENSOR_RESET_CTX(dev) (g_pastOv6946[dev] = HI_NULL)
...
ISP_SNS_COMMBUS_U g_aunOv6946BusInfo[ISP_MAX_PIPE_NUM] = {
...
typedef struct hiOV6946_STATE_S {
    HI_U32 u32BRL;
    HI_U32 u32RHS1_MAX;
    HI_U32 u32deltaRHS1;
} OV6946_STATE_S;

OV6946_STATE_S g_astov6946State[ISP_MAX_PIPE_NUM] = { { 0 } };


/****************************************************************************
 * extern                                                                   *
 ****************************************************************************/
extern unsigned char ov6946_i2c_addr;
extern unsigned int ov6946_addr_byte;
extern unsigned int ov6946_data_byte;

extern void ov6946_init(VI_PIPE ViPipe);
extern void ov6946_exit(VI_PIPE ViPipe);
extern void ov6946_standby(VI_PIPE ViPipe);
extern void ov6946_restart(VI_PIPE ViPipe);
extern int ov6946_write_register(VI_PIPE ViPipe, int addr, int data);
extern int ov6946_read_register(VI_PIPE ViPipe, int addr);

/****************************************************************************
 * local variables                                                            *
 ****************************************************************************/
#define OV6946_FULL_LINES_MAX               (0xFFFF) /* VMAX is 20bit */ //OV中对应的寄存器是VTS
#define OV6946_FULL_LINES_MAX_2TO1_WDR      (0x1194)  // considering the YOUT_SIZE and bad frame

#define OV6946_INCREASE_LINES               (0) /* make real fps less than stand fps because NVR require */

#define OV6946_VMAX_1M_30FPS_LINEAR         (812)
#define OV6946_VMAX_1M_30FPS_2TO1_WDR       (812)

static HI_U32 gu32FullLinesStd = OV6946_VMAX_1M_30FPS_LINEAR;
//static HI_U32 gu32FullLines = OV6946_VMAX_1M_30FPS_LINEAR;

// sensor fps mode
#define OV6946_1M_30FPS_LINEAR_MODE   (1) /* 1K@30fps Linear 12bit */
#define OV6946_1M_30FPS_2t1_DOL_MODE  (2)

#define OV6946_RES_IS_1K30_10BIT(w, h) (((w) == 400) && ((h) == 400))

// sensor gain
#define OV6946_AGAIN_MAX                    (31356)  // the max again is 31356
#define OV6946_DGAIN_MAX                    (124833)  // the max dgain is 124833
...
static HI_S32 cmos_get_ae_default(VI_PIPE ViPipe, AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    ...
    OV6946_SENSOR_GET_CTX(ViPipe, pstSnsState);
    ...
    pstAeSnsDft->u32FullLinesMax = OV6946_FULL_LINES_MAX;
    ...
    if (g_au32InitExposure[ViPipe] == 0) {
        pstAeSnsDft->u32InitExposure = 100000;
    }
    ...
    switch (pstSnsState->enWDRMode) {
        ...
        case WDR_MODE_NONE:
            ...
            pstAeSnsDft->u32HmaxTimes = (1000000 * 1000) / (OV6946_VMAX_1M_30FPS_LINEAR * 30);
...
static HI_VOID cmos_fps_set(VI_PIPE ViPipe, HI_FLOAT f32Fps, AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    ...
    switch (pstSnsState->u8ImgMode) {             //在sensor_global_init中初始化
        case OV6946_1M_30FPS_LINEAR_MODE:
            u32MaxFps = 30;
            if ((f32Fps <= 30) && (f32Fps >= 0.07)) {
                u32Lines = OV6946_VMAX_1M_30FPS_LINEAR * u32MaxFps / DIV_0_TO_1_FLOAT(f32Fps);
                pstAeSnsDft->u32LinesPer500ms = OV6946_VMAX_1M_30FPS_LINEAR*15;
            } else {
                ISP_ERR_TRACE("Not support Fps: %f\n", f32Fps);
                return;
            }
            u32Lines = (u32Lines > OV6946_FULL_LINES_MAX) ? OV6946_FULL_LINES_MAX : u32Lines;
            break;

        case OV6946_1M_30FPS_2t1_DOL_MODE:
            u32MaxFps = 30;
            if ((f32Fps <= 30) && (f32Fps >= 15)) {
                u32Lines = OV6946_VMAX_1M_30FPS_2TO1_WDR * u32MaxFps / DIV_0_TO_1_FLOAT(f32Fps);
                pstAeSnsDft->u32LinesPer500ms = OV6946_VMAX_1M_30FPS_2TO1_WDR*30;
            } else {
                ISP_ERR_TRACE("Not support Fps: %f\n", f32Fps);
                return;
            }
            u32Lines = u32Lines + (u32Lines % 2);  // VMAX is 2n(n-0,1,2.....)
            u32Lines = (u32Lines > OV6946_FULL_LINES_MAX_2TO1_WDR) ? OV6946_FULL_LINES_MAX_2TO1_WDR : u32Lines;
            break;
        default:
    ...
    if (pstSnsState->enWDRMode == WDR_MODE_2To1_LINE) {
        ...
        g_astov6946State[ViPipe].u32RHS1_MAX = g_astov6946State[ViPipe].u32BRL * 2;
        g_astov6946State[ViPipe].u32deltaRHS1 = u32Lines * 2 - g_astov6946State[ViPipe].u32BRL * 2 - 2;
    } else {
...
static HI_VOID cmos_slow_framerate_set(VI_PIPE ViPipe, HI_U32 u32FullLines, AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    ...
    if (pstSnsState->enWDRMode == WDR_MODE_2To1_LINE) {
        u32FullLines = (u32FullLines > 2 * OV6946_FULL_LINES_MAX_2TO1_WDR) ? 2 * OV6946_FULL_LINES_MAX_2TO1_WDR : u32FullLines;
        pstSnsState->au32FL[0] = (u32FullLines >> 1) << 1;
        g_astov6946State[ViPipe].u32RHS1_MAX = 2 * 2200;
        g_astov6946State[ViPipe].u32deltaRHS1 = u32FullLines - g_astov6946State[ViPipe].u32BRL * 2 - 2;
    } else {
        u32FullLines = (u32FullLines > OV6946_FULL_LINES_MAX) ? OV6946_FULL_LINES_MAX : u32FullLines;
    ...
    if (pstSnsState->enWDRMode == WDR_MODE_2To1_LINE) {
        ...
        /* long exposure */
            ...
            if (u32RHS1[ViPipe] >= u32LastRHS1[ViPipe]) {
                ...
                if (u32deltaRHS1[ViPipe] <= g_astov6946State[ViPipe].u32deltaRHS1) {
                } else {
                    if (u32ShortIntTime[ViPipe] % 4 == 0) {
                        u32SHR1[ViPipe] = 9 + (u32deltaRHS1[ViPipe] - g_astov6946State[ViPipe].u32deltaRHS1 + 2);
                        u32RHS1[ViPipe] = u32LastRHS1[ViPipe] - (g_astov6946State[ViPipe].u32deltaRHS1 - 2);
                    } else if (u32ShortIntTime[ViPipe] % 4 == 2) {
                        u32SHR1[ViPipe] = 11 + (u32deltaRHS1[ViPipe] - g_astov6946State[ViPipe].u32deltaRHS1 + 2);
                        u32RHS1[ViPipe] = u32LastRHS1[ViPipe] - (g_astov6946State[ViPipe].u32deltaRHS1 - 2);
                    }
                }
        ...
        pstSnsState->astRegsInfo[0].astI2cData[0].u32Data = (u32IntTime >> 12) & 0xF;
        pstSnsState->astRegsInfo[0].astI2cData[1].u32Data = (u32IntTime >> 4) & 0xFF;
        pstSnsState->astRegsInfo[0].astI2cData[2].u32Data = (u32IntTime & 0xF) << 4;
...
static HI_VOID cmos_get_inttime_max(VI_PIPE ViPipe, HI_U16 u16ManRatioEnable, HI_U32 *au32Ratio, HI_U32 *au32IntTimeMax, HI_U32 *au32IntTimeMin, HI_U32 *pu32LFMaxIntTime)
{
    ...
    if (pstSnsState->enWDRMode == WDR_MODE_2To1_LINE) {
        ...
        if (genFSWDRMode[ViPipe] == ISP_FSWDR_LONG_FRAME_MODE) {
            ...
            u32IntTimeMaxTmp = (u32IntTimeMaxTmp0 < u32IntTimeMaxTmp) ? u32IntTimeMaxTmp0 : u32IntTimeMaxTmp;
            u32IntTimeMaxTmp = (u32IntTimeMaxTmp > (g_astov6946State[ViPipe].u32RHS1_MAX - 2)) ? (g_astov6946State[ViPipe].u32RHS1_MAX - 2) : u32IntTimeMaxTmp;
...
static HI_S32 cmos_get_isp_default(VI_PIPE ViPipe, ISP_CMOS_DEFAULT_S *pstDef)
{
    ...
    pstDef->stSensorMode.u32SensorID = OV6946_ID;
...
static HI_VOID cmos_set_pixel_detect(VI_PIPE ViPipe, HI_BOOL bEnable)
{
    ...
    if (pstSnsState->enWDRMode == WDR_MODE_2To1_LINE) {
        u32FullLines_5Fps = (OV6946_VMAX_1M_30FPS_2TO1_WDR * 30) / 5;
    } else {
        if (pstSnsState->u8ImgMode == OV6946_1M_30FPS_LINEAR_MODE) {

            u32FullLines_5Fps = (OV6946_VMAX_1M_30FPS_LINEAR * 30) / 5;
            pstSnsState->u32FLStd = (pstSnsState->u32FLStd > OV6946_FULL_LINES_MAX) ? OV6946_FULL_LINES_MAX : pstSnsState->u32FLStd;
    ...
    if (bEnable) { /* setup for ISP pixel calibration mode */
        ...
        //ov6946_write_register(ViPipe, 0x5182, 0x01);
    #if 0
        ov6946_write_register(ViPipe,0x380e, (u32FullLines_5Fps & 0xFF00) >> 8);  /* 5fps */
        ov6946_write_register(ViPipe,0x380f, u32FullLines_5Fps & 0xFF);           /* 5fps */
    #endif
        ov6946_write_register(ViPipe,0x3500, (u32MaxIntTime_5Fps >> 12));               /* max exposure lines */
        ov6946_write_register(ViPipe,0x3500+1, ((u32MaxIntTime_5Fps & 0xFFF)>>4));   /* max exposure lines */
        ov6946_write_register(ViPipe,0x3500+2, ((u32MaxIntTime_5Fps & 0xF)<<4));       /* max exposure lines */ 
        ov6946_write_register(ViPipe,0x350a, 0x00);                                    /* min AG */
        ov6946_write_register(ViPipe,0x350a+1, 0x10);                                    /* min AG */
    
    } else { /* setup for ISP 'normal mode' */
        pstSnsState->u32FLStd = (pstSnsState->u32FLStd > OV6946_FULL_LINES_MAX) ? OV6946_FULL_LINES_MAX : pstSnsState->u32FLStd;
        ov6946_write_register(ViPipe,0x380e, (gu32FullLinesStd & 0xFF00) >> 8);
        ov6946_write_register(ViPipe,0x380f, gu32FullLinesStd & 0xFF);
...
static HI_S32 cmos_get_sns_regs_info(VI_PIPE ViPipe, ISP_SNS_REGS_INFO_S *pstSnsRegsInfo)
{
    ...
    if ((pstSnsState->bSyncInit == HI_FALSE) || (pstSnsRegsInfo->bConfig == HI_FALSE)) {
        ...
        pstSnsState->astRegsInfo[0].unComBus.s8I2cDev = g_aunOv6946BusInfo[ViPipe].s8I2cDev;
        ...
        for (i = 0; i < pstSnsState->astRegsInfo[0].u32RegNum; i++) {
            ...
            pstSnsState->astRegsInfo[0].astI2cData[i].u8DevAddr = ov6946_i2c_addr;
            pstSnsState->astRegsInfo[0].astI2cData[i].u32AddrByteNum = ov6946_addr_byte;
            pstSnsState->astRegsInfo[0].astI2cData[i].u32DataByteNum = ov6946_data_byte;
        }

        pstSnsState->astRegsInfo[0].astI2cData[0].u8DelayFrmNum = 0;         //exposure time: astI2cData[0:2]
        pstSnsState->astRegsInfo[0].astI2cData[0].u32RegAddr = 0x3500;
    
        pstSnsState->astRegsInfo[0].astI2cData[1].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[1].u32RegAddr = 0x3501;

        pstSnsState->astRegsInfo[0].astI2cData[2].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[2].u32RegAddr = 0x3502;

        //pstSnsState->astRegsInfo[0].astI2cData[3].u8DelayFrmNum = 0;       //gain
        //pstSnsState->astRegsInfo[0].astI2cData[3].u32RegAddr = 0x350a;

        pstSnsState->astRegsInfo[0].astI2cData[3].u8DelayFrmNum = 0;       //gain
        pstSnsState->astRegsInfo[0].astI2cData[3].u32RegAddr = 0x350b;
#if 0

        pstSnsState->astRegsInfo[0].astI2cData[4].u8DelayFrmNum = 0;       //gain
        pstSnsState->astRegsInfo[0].astI2cData[4].u32RegAddr = 0x5180;
        pstSnsState->astRegsInfo[0].astI2cData[5].u8DelayFrmNum = 0;       //gain
        pstSnsState->astRegsInfo[0].astI2cData[5].u32RegAddr = 0x5181;

        pstSnsState->astRegsInfo[0].astI2cData[6].u8DelayFrmNum = 0;         
        pstSnsState->astRegsInfo[0].astI2cData[6].u32RegAddr = 0x5182;
        pstSnsState->astRegsInfo[0].astI2cData[7].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[7].u32RegAddr = 0x5183;

        pstSnsState->astRegsInfo[0].astI2cData[8].u8DelayFrmNum = 0;         
        pstSnsState->astRegsInfo[0].astI2cData[8].u32RegAddr = 0x5184;
        pstSnsState->astRegsInfo[0].astI2cData[9].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[9].u32RegAddr = 0x5185;

        pstSnsState->astRegsInfo[0].astI2cData[6].u8DelayFrmNum = 0;         //VTS
        pstSnsState->astRegsInfo[0].astI2cData[6].u32RegAddr = 0x380e;
        pstSnsState->astRegsInfo[0].astI2cData[7].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[7].u32RegAddr = 0x380f;
...
static HI_S32 cmos_set_image_mode(VI_PIPE ViPipe, ISP_CMOS_SENSOR_IMAGE_MODE_S *pstSensorImageMode)
{
    ...
    if (f32Fps <= 30) {
        if (pstSnsState->enWDRMode == WDR_MODE_NONE) {
            if (OV6946_RES_IS_1K30_10BIT(u32W, u32H)) {
                u8SensorImageMode = OV6946_1M_30FPS_LINEAR_MODE;
                pstSnsState->u32FLStd = OV6946_VMAX_1M_30FPS_LINEAR;
            } else {
            ...
            if (OV6946_RES_IS_1K30_10BIT(u32W, u32H)) {
                u8SensorImageMode = OV6946_1M_30FPS_2t1_DOL_MODE;
                g_astov6946State[ViPipe].u32BRL = 2200;
                pstSnsState->u32FLStd = OV6946_VMAX_1M_30FPS_2TO1_WDR * 2;
            } else {
                ISP_ERR_TRACE("Not support! Width:%d, Height:%d, Fps:%f, WDRMode:%d\n",
                          pstSensorImageMode->u16Width,
                          pstSensorImageMode->u16Height,
...
static HI_VOID sensor_global_init(VI_PIPE ViPipe)
{
    ...
    pstSnsState->bSyncInit = HI_FALSE;
    pstSnsState->u8ImgMode = OV6946_1M_30FPS_LINEAR_MODE;
    pstSnsState->enWDRMode = WDR_MODE_NONE;
    pstSnsState->u32FLStd = OV6946_1M_30FPS_LINEAR_MODE;
    pstSnsState->au32FL[0] = OV6946_1M_30FPS_LINEAR_MODE;
    pstSnsState->au32FL[1] = OV6946_1M_30FPS_LINEAR_MODE;
...
static HI_S32 cmos_init_sensor_exp_function(ISP_SENSOR_EXP_FUNC_S *pstSensorExpFunc)
{
    ...
    pstSensorExpFunc->pfn_cmos_sensor_init = ov6946_init;
    pstSensorExpFunc->pfn_cmos_sensor_exit = ov6946_exit;
...
static HI_S32 ov6946_set_bus_info(VI_PIPE ViPipe, ISP_SNS_COMMBUS_U unSNSBusInfo)
{
    g_aunOv6946BusInfo[ViPipe].s8I2cDev = unSNSBusInfo.s8I2cDev;
    return HI_SUCCESS;
}
...
static HI_S32 sensor_ctx_init(VI_PIPE ViPipe)
{
    ...
    OV6946_SENSOR_GET_CTX(ViPipe, pastSnsStateCtx);
    ...
    OV6946_SENSOR_SET_CTX(ViPipe, pastSnsStateCtx);
...
static HI_VOID sensor_ctx_exit(VI_PIPE ViPipe)
{
    ...
    OV6946_SENSOR_RESET_CTX(ViPipe);
}
...
static HI_S32 sensor_register_callback(VI_PIPE ViPipe, ALG_LIB_S *pstAeLib, ALG_LIB_S *pstAwbLib)
{
    ...
    stSnsAttrInfo.eSensorId = OV6946_ID;
...
static HI_S32 sensor_unregister_callback(VI_PIPE ViPipe, ALG_LIB_S *pstAeLib, ALG_LIB_S *pstAwbLib)
{
    ...
    s32Ret = HI_MPI_ISP_SensorUnRegCallBack(ViPipe, OV6946_ID);
    ...
    s32Ret = HI_MPI_AE_SensorUnRegCallBack(ViPipe, pstAeLib, OV6946_ID);
    ...
    s32Ret = HI_MPI_AWB_SensorUnRegCallBack(ViPipe, pstAwbLib, OV6946_ID);
...
ISP_SNS_OBJ_S stSnsOv6946Obj = {
    .pfnRegisterCallback = sensor_register_callback,
    .pfnUnRegisterCallback = sensor_unregister_callback,
    .pfnStandby = ov6946_standby,
    .pfnRestart = ov6946_restart,
    .pfnMirrorFlip = HI_NULL,
    .pfnWriteReg = ov6946_write_register,
    .pfnReadReg = ov6946_read_register,
    .pfnSetBusInfo = ov6946_set_bus_info,
