...
{
    mipi_tx_dev_phy_t* pmipi_tx_phy_ctx = NULL;
    pmipi_tx_phy_ctx = &mipi_tx_phy_ctx;

    /* mipi tx phy status */
    osal_seq_printf(s, "----------MIPI_Tx DEV STATUS---------------------------\n");
    osal_seq_printf(s, "%8s%8s%8s%8s%8s%8s%8s\n",
        "width", "height", "HoriAll", "VertAll", "hbp", "hsa", "vsa");
    osal_seq_printf(s, "%8d%8d%8d%8d%8d%8d%8d\n",
        pmipi_tx_phy_ctx->hact_det,
        pmipi_tx_phy_ctx->vact_det,
        pmipi_tx_phy_ctx->hall_det,
        pmipi_tx_phy_ctx->vall_det,
        pmipi_tx_phy_ctx->hbp_det,
        pmipi_tx_phy_ctx->hsa_det,
        pmipi_tx_phy_ctx->vsa_det);
    osal_seq_printf(s, "\r\n");
    return 0;
}
