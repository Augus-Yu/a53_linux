...
void hi_user_notify_ao_event(int aodev)
{
    switch (aodev)
    {
        case 0:
...
int __init hi_user_init(void)
{
...
#if VO_NOTIFY
    {
        ...
        if(NULL != g_vou_exp_symbol.pfnVoRegisterExpCallback)
        {
            stVoExpCallback.pfnVoNotify = hi_user_notify_vo_event;
            g_vou_exp_symbol.pfnVoRegisterExpCallback(&stVoExpCallback);
        }
    }
#endif
...
#if AO_NOTIFY
    {
        AO_EXPORT_CALLBACK_S stAoExpCallback;
        memset(&stAoExpCallback, 0, sizeof(AO_EXPORT_CALLBACK_S));

        if (NULL != g_stAoExpSymbol.pfnAoRegisterExpCallback)
        {
            stAoExpCallback.pfnAoNotify = hi_user_notify_ao_event;
            g_stAoExpSymbol.pfnAoRegisterExpCallback(&stAoExpCallback);
        }
    }
#endif
