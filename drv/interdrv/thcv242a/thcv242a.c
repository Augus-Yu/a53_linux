...
static int thcv242a_init_pre(struct thcv242a_priv *priv)
{
    ...
    printk("[YDY]i2c_addr: %x\n",priv->client->addr);
    //err |= thcv242a_write(priv,R_2WIREPT1_PASS_ADRIN0,priv->client->addr);
    err |= thcv242a_write(priv,0x0010,0x00);
    err |= thcv242a_write(priv,0x1005,0x01);
    err |= thcv242a_write(priv,0x100d,0x02);
    err |= thcv242a_write(priv,0x1704,0x00);
    ...
    err |= thcv242a_write(priv,0x1704,0x10);
			   ...
			   const struct i2c_device_id *id)
{
	...
	struct device *dev = &client->dev;
	int err;
    printk("[YDY]Into thcv242a_probe\n");
...
static const struct i2c_device_id thcv242a_id[] = 
{
	//{ "thcv242a", 0 },
	{/* sentinel */}
};
MODULE_DEVICE_TABLE(i2c,thcv242a_id);

static const struct of_device_id thcv242a_dt_ids[] = {
	{ .compatible = "weigao,thcv242a" },
	{/* sentinel */}
};
MODULE_DEVICE_TABLE(of, thcv242a_dt_ids);

static struct i2c_driver thcv242a_driver = {
	.driver = {
        .owner = THIS_MODULE,
		.name = "thcv242a",
		.of_match_table = thcv242a_dt_ids,
	},
	.probe = thcv242a_probe,
	.remove = thcv242a_remove,
    .id_table	= thcv242a_id,
};
