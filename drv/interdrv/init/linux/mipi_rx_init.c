...
extern unsigned int g_slvs_irq_num;
extern void *g_slvs_regs_va;
...
static int hi35xx_mipi_rx_probe(struct platform_device *pdev)
{
    ...
    mem = platform_get_resource_byname(pdev, IORESOURCE_MEM, "slvs");
    g_slvs_regs_va = devm_ioremap_resource(&pdev->dev, mem);
    if (IS_ERR(g_slvs_regs_va))
    {
        dev_err(&pdev->dev, "slvs remap mem error.\n");
        return PTR_ERR(g_slvs_regs_va);
    }

    g_slvs_irq_num = platform_get_irq_byname(pdev, "slvs");
    if (g_slvs_irq_num <= 0)
    {
        dev_err(&pdev->dev, "can not find slvs IRQ\n");
    }
