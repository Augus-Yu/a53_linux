...
#include "hi_sil9022.h"
#include "stdio.h"
#undef DEBUG_LEVEL
#endif
#define DEV_NAME "sil9022"
...
{
    I2C_BOARD_INFO("sil9022", TX_SLAVE_ADDR >> 1),
};
...
{
    int ret = 0;
    struct i2c_client * i2c_client0 = &sil_client_obj;
    i2c_client0->addr = TX_SLAVE_ADDR >> 1;
    ret = client_attach(i2c_client0, SIL9022_I2C_NUM);
    if(ret) {
...
void hi_sil_i2c_client_deinit(void)
{
    struct i2c_client * i2c_client0 = &sil_client_obj;
...
static int i2c_client_init(void)
{
    ...
    // use i2c0
    i2c_adap   = i2c_get_adapter(SIL9022_I2C_NUM);
    //printk("i2c_adap = %p\n",i2c_adap);
    if (NULL != i2c_adap)
    {
...
{
#ifdef DRV_I2C
    return drv_i2c_read(SIL9022_I2C_NUM, devaddress, address, 1, 1);
#else
    ...
    //printk("client->flags=%d\n",client->flags);
    //printk("client->name=%s\n",client->name);
    //printk("devaddress=0x%x\n",devaddress);
    //printk("address=0x%x\n",address);
    //printk("nnnnnnnnnnnnnnnnnnnnnnnnnnn\n");
    buffer[0] = address;
...
#ifdef DRV_I2C
    return drv_i2c_write(SIL9022_I2C_NUM, devaddress, address, 1, data, 1);
#else
    int                   ret;
    unsigned char         buf[2];
    struct i2c_client*    client = sil_client;
...
void SetFormat (byte* Data)
{
    ...
    //printk(KERN_INFO "sil9022 SetFormat OUTPUT_MODE_HDMI\n");
    ...
    if (!IsHDMI_Sink())
    {
        ...
        TPI_DEBUG_PRINT(("sil9022 SetFormat OUTPUT_MODE_DVI....\n"));
    }
...
static int sil902x_device_init(void)
{
    ...
    msleep(1000); // why? I have to add the time because the following I2C is invalid without it. 9022 need to be stable??
...
int sil902x_init(void* pArgs)
{
    ...
    ret = register_driver("/dev/sil9022", &sil902x_fops, 0666, 0);
    if (ret)
    {
        DPRINTK(0, "could not register sil9022 device");
        goto OUT0;
    }

    if(-1 == i2c_client_init())
    {
        DPRINTK(0, "sil9xxx : i2c_client_init failed\n");

        goto OUT1;
    }

    /* sleep 3s to wait i2c driver ok! */
    sleep(3);

    if (sil902x_device_init() < 0)
    {
        DPRINTK(0, "sil9xxx : sil902x_device_init failed\n");
        goto OUT2;
    }
...
OUT2:
    i2c_client_deinit();
OUT1:
    unregister_driver("/dev/sil9022");
OUT0:
    DPRINTK(0, "load sil9xxx failed\n");
    return -1;
}
...
static int __init sil902x_init(void)
{
    ...
    {
        DPRINTK(0, "sil902x client init failed!");
#ifdef CONFIG_HISI_SNAPSHOT_BOOT
    himedia_unregister(&s_stSil902xDevice);
#else
    misc_deregister(&sil902x_dev);
